...
public class ChoosePictureOrVideo {
    ...
    public static ChoosePictureOrVideo getInstance(){
        if (pictureOrVideo == null){
            synchronized (ChoosePictureOrVideo.class){
                if (pictureOrVideo == null){
    ...
    //页面跳转  选择的是图片还是视频
    public void ChoosePictureOrVideo(Activity context,int selectType, boolean isSingle,List<LocalMedia> selectMedia){

        int maxSelectNum = 1;//最大选择数量
        int selectionMode = PictureConfig.MULTIPLE;//单选 or 多选
        ...
        switch (selectType){
            ...
            case 2:
                ...
                PictureSelector.create(context)
                        ...
                        .videoSecond(11)//显示多少秒以内的视频or音频也可适用
    ...
    //页面跳转  选择的是图片还是视频  这个是用于发布任务时  选择图片   需要手动传入requestCode
    public void ChoosePictureOrVideo(Activity context,int selectType, boolean isSingle,List<LocalMedia> selectMedia,int requestCode){
